...
@JvmInline value class Personident(
    val id: String,
) {
    init {
        check(id.isNotBlank()) { "Ugyldig personident!" }
    }
}

@JvmInline value class Inntektskilde(
    val id: String,
) {
    init {
        check(id.isNotBlank()) { "Ugyldig inntektskilde!" }
    }
}
...
data class BeregnetDaglig(
    private val periodisert: Beløp.Periodisert,
) {
    ...
    private val vektlagteVirkedager =
        periodisert.periode
